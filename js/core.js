// js/core.js — الدوال الأساسية مع apiCall المحلي
import { get as storeGet, set as storeSet, invalidate } from './store.js';
import { apiCall as dbApiCall } from './db.js';
...
export async function apiCall(endpoint, method = 'GET', body = {}) {
  ...
  try {
    const result = await dbApiCall(endpoint, method, body);

    if (method === 'GET' && result !== undefined) {
      storeSet(storeKey, result);
    }

    if (method === 'POST' || method === 'PUT' || method === 'DELETE') {
      invalidate(getEntityFromEndpoint(endpoint));
    }

    return result;
  } catch (err) {
    throw new Error(err.message || 'حدث خطأ أثناء العملية');
  }
}
...
export function renderSkeleton(type = 'cards') {
  ...
  switch (type) {
    case 'cards':
      ...
      break;
    case 'table':
      html = `<div class="skeleton-table"><div class="skeleton-header">${Array(4).fill('<div class="skeleton-line w-25"></div>').join('')}</div>${Array(5).fill(`<div class="skeleton-row">${Array(4).fill('<div class="skeleton-line"></div>').join('')}</div>`).join('')}</div>`;
      break;
    case 'stats':
