...
async function loadUnitsSection(){
  try{
    ...
    document.querySelectorAll('.delete-unit-btn').forEach(b=>b.addEventListener('click',async e=>{ if(await confirmDialog('حذف الوحدة؟')){ await apiCall('/definitions?type=unit&id='+b.dataset.id,'DELETE'); loadUnitsSection(); } }));
  }catch(e){ showToast(e.message,'error'); }
}
function showAddUnitModal(){ showFormModal({title:'إضافة وحدة',fields:[{id:'name',label:'الاسم'},{id:'abbreviation',label:'الاختصار'}],onSave:async v=>{ if(!v.name) throw new Error('الاسم مطلوب'); return apiCall('/definitions?type=unit','POST',v); },onSuccess:loadUnitsSection}); }
function showEditUnitModal(id){ const u=unitsCache.find(x=>x.id==id); if(!u) return; showFormModal({title:'تعديل وحدة',fields:[{id:'name',label:'الاسم'},{id:'abbreviation',label:'الاختصار'}],initialValues:u,onSave:async v=>apiCall('/definitions?type=unit','PUT',{id,...v}),onSuccess:loadUnitsSection}); }
...
function showAddItemModal(){
  ...
  toggleBtn.onclick=()=>{ extraUnitsDiv.style.display=(extraUnitsDiv.style.display==='none'?'block':'none'); toggleBtn.innerHTML=(extraUnitsDiv.style.display==='block'?ICONS.x:ICONS.plus); };
  function updateQty(){
    ...
    if(qty>0 && unit!=='base'){ qtyConverted.style.display='block'; qtyBaseVal.textContent=baseQty; } else { qtyConverted.style.display='none'; }
  }
  ...
  modal.element.querySelector('#btn-save-quick-cat').onclick=async ()=>{
    ...
    try{
      ...
      input.value=''; modal.element.querySelector('#quick-cat-row').style.display='none';
  ...
  modal.element.querySelector('#fm-save').onclick=async ()=>{
    ...
    if(!name) return showToast('اسم المادة مطلوب','error');
    ...
    if(u2name && u2factor>0){ const uid=await getOrCreateUnit(u2name); if(uid) item_units.push({unit_id:uid, conversion_factor:u2factor}); }
    ...
    if(u3name && u3factor>0){ const uid=await getOrCreateUnit(u3name); if(uid) item_units.push({unit_id:uid, conversion_factor:u3factor}); }
...
async function loadGenericSection(endpoint,cacheKey){
  ...
  else data.forEach(item=>{ html+=`<div class="card card-hover" style="display:flex;justify-content:space-between;align-items:center;"><span style="font-weight:800;">${item.name}</span><div><button class="btn btn-secondary btn-sm edit-btn" data-id="${item.id}" data-type="${cacheKey}">${ICONS.edit}</button> <button class="btn btn-danger btn-sm delete-btn" data-id="${item.id}" data-type="${cacheKey}">${ICONS.trash}</button></div></div>`; });
  tc.innerHTML=html;
}
...
function showFormModal({title,fields,initialValues={},onSave,onSuccess}){
  ...
  let body='';
  fields.forEach(f=>{ const val=initialValues[f.id]!==undefined?initialValues[f.id]:''; body+=`<div class="form-group"><label class="form-label">${f.label}</label><input class="input" id="${formId}-${f.id}" type="${f.type||'text'}" value="${val}"></div>`; });
  ...
  modal.element.querySelector(`#${formId}-save`).onclick=async ()=>{
    const values={}; fields.forEach(f=>{ const el=modal.element.querySelector(`#${formId}-${f.id}`); values[f.id]=el?el.value.trim():''; });
    try{ await onSave(values); modal.close(); showToast('تم الحفظ','success'); if(onSuccess) onSuccess(); }catch(e){ showToast(e.message,'error'); }
  };
}
...
async function showInvoiceModal(type){
  try{
    ...
    const updateGrandTotal=()=>{ let t=0; container.querySelectorAll('.total-input').forEach(inp=>t+=parseFloat(inp.value)||0); const gt=container.querySelector('#inv-grand-total'); if(gt) gt.textContent=formatNumber(t); };
    const calc=row=>{ const qty=parseFloat(row.querySelector('.qty-input')?.value)||0; const price=parseFloat(row.querySelector('.price-input')?.value)||0; const tot=row.querySelector('.total-input'); if(tot){ tot.value=(qty*price).toFixed(2); updateGrandTotal(); } };
    const getUnitOptions=item=>{
      if(!item) return '<option value="">اختر مادة</option>';
      const baseUnit=unitsCache.find(u=>u.id==item.base_unit_id)||{}; const baseName=baseUnit.name||'قطعة';
      let opts=`<option value="" data-factor="1">${baseName} (أساسية)</option>`;
      (item.item_units||[]).forEach(iu=>{ const unit=unitsCache.find(u=>u.id==iu.unit_id)||{}; opts+=`<option value="${iu.unit_id}" data-factor="${iu.conversion_factor}">${unit.name||unit.abbreviation||'وحدة'} (×${iu.conversion_factor})</option>`; });
      return opts;
    };
    const rowHandler=row=>{
      ...
      sel.addEventListener('change',()=>{
        ...
        if(item){ const basePrice=type==='sale'?(item.selling_price||0):(item.purchase_price||0); pr.value=basePrice; if(unitSel){ unitSel.innerHTML=getUnitOptions(item); unitSel.style.display='block'; unitSel.dataset.basePrice=basePrice; } calc(row); }
        else{ pr.value=''; if(unitSel){ unitSel.innerHTML='<option value="">الوحدة</option>'; unitSel.style.display='none'; } }
      });
      row.querySelector('.qty-input')?.addEventListener('input',()=>calc(row));
      row.querySelector('.price-input')?.addEventListener('input',()=>calc(row));
      unitSel?.addEventListener('change',()=>{ const factor=parseFloat(unitSel.selectedOptions[0]?.dataset.factor||1); const basePrice=parseFloat(unitSel.dataset.basePrice||0); pr.value=(basePrice*factor).toFixed(2); calc(row); });
    };
    ...
    container.querySelector('#btn-add-line').addEventListener('click',()=>{
      ...
      container.querySelector('#inv-lines').appendChild(nl); rowHandler(nl);
    ...
    modal.element.querySelector('#inv-save').onclick=async ()=>{
      const lines=[]; container.querySelectorAll('.line-row').forEach(row=>{ const id=row.querySelector('.item-select')?.value||null; const unitId=row.querySelector('.unit-select')?.value||null; const factor=parseFloat(row.querySelector('.unit-select')?.selectedOptions[0]?.dataset.factor||1); const qty=parseFloat(row.querySelector('.qty-input')?.value)||0; const price=parseFloat(row.querySelector('.price-input')?.value)||0; const total=parseFloat(row.querySelector('.total-input')?.value)||0; if(id||qty>0) lines.push({item_id:id,unit_id:unitId,quantity:qty,unit_price:price,total,conversion_factor:factor}); });
      ...
      await apiCall('/invoices','POST',{
        ...
        lines, total:lines.reduce((s,l)=>s+l.total,0), paid_amount:paid
      });
...
function renderFilteredInvoices(){
  ...
  let data=invoicesCache; if(filt!=='all') data=data.filter(inv=>inv.type===filt);
  ...
  container.querySelectorAll('.view-inv-btn').forEach(b=>b.addEventListener('click',e=>{ const inv=invoicesCache.find(i=>i.id==parseInt(b.dataset.id)); if(inv) showInvoiceDetail(inv); }));
  container.querySelectorAll('.print-inv-btn').forEach(b=>b.addEventListener('click',e=>{ const inv=invoicesCache.find(i=>i.id==parseInt(b.dataset.id)); if(inv) window.printInvoice(inv,{preview:true,format:'thermal'}); }));
  container.querySelectorAll('.delete-inv-btn').forEach(b=>b.addEventListener('click',async e=>{ if(await confirmDialog('حذف الفاتورة؟')){ await apiCall('/invoices?id='+b.dataset.id,'DELETE'); loadInvoices(); } }));
}
...
window.printInvoice = function(invoice, options={}){
  ...
  const w=window.open('','_blank'); w.document.write(thermalHTML); w.document.close(); setTimeout(()=>w.print(),500);
};
  ...
  async function loadPayments(){
    ...
    payments.forEach(p=>{ const isIn=!!p.customer_id; tc.innerHTML+=`<div class="card" style="border-right:3px solid ${isIn?'var(--success)':'var(--danger)'};"><div style="font-weight:900;font-size:20px;color:${isIn?'var(--success)':'var(--danger)'};">${isIn?'+':'-'} ${formatNumber(p.amount)}</div><div style="font-size:13px;color:var(--text-muted);">${formatDate(p.payment_date)} · ${p.notes||''}</div></div>`; });
  ...
  function showAddPaymentModal(){
    ...
    modal.element.querySelector('#pmt-save').onclick=async ()=>{
      const type=modal.element.querySelector('#pmt-type').value; const amount=parseFloat(modal.element.querySelector('#pmt-amount').value); if(!amount) return showToast('المبلغ مطلوب','error');
  ...
  async function loadExpenses(){
    ...
    document.getElementById('btn-add-exp')?.addEventListener('click',()=>{ showFormModal({title:'إضافة مصروف',fields:[{id:'amount',label:'المبلغ',type:'number'},{id:'expense_date',label:'التاريخ',type:'date'},{id:'description',label:'الوصف'}],initialValues:{expense_date:new Date().toISOString().split('T')[0]},onSave:v=>apiCall('/expenses','POST',v),onSuccess:loadExpenses}); });
  }

  // ===== التقارير (مجموعة كاملة) =====
  ...
  async function loadTrialBalance(){
    ...
    const html=`<div class="card"><button class="btn btn-secondary btn-sm" onclick="loadReports()">🔙 رجوع</button><h3>ميزان المراجعة</h3>
  ...
  async function loadIncomeStatement(){
    ...
    document.getElementById('tab-content').innerHTML=`<div class="card"><button class="btn btn-secondary btn-sm" onclick="loadReports()">🔙 رجوع</button><h3>قائمة الدخل</h3>
    <p>الإيرادات: ${formatNumber(totalSales)}</p><p>تكلفة المبيعات: ${formatNumber(totalPurchases)}</p><p>المصاريف: ${formatNumber(totalExpenses)}</p><hr><h2 style="color:${net>=0?'var(--success)':'var(--danger)'}">صافي الربح: ${formatNumber(net)}</h2></div>`;
  }
  ...
  async function loadBalanceSheet(){
    ...
    document.getElementById('tab-content').innerHTML=`<div class="card"><button class="btn btn-secondary btn-sm" onclick="loadReports()">🔙 رجوع</button><h3>الميزانية</h3>
  ...
  async function loadCustomerStatementForm(){
    ...
    document.getElementById('tab-content').innerHTML=`<div class="card"><button class="btn btn-secondary btn-sm" onclick="loadReports()">🔙 رجوع</button><h3>كشف حساب عميل</h3><select class="select" id="cust-sel">${opts}</select><button class="btn btn-primary" id="btn-show-cust">عرض</button><div id="stmt-result"></div></div>`;
    document.getElementById('btn-show-cust').onclick=async ()=>{
      ...
      html+='</tbody></table></div>'; document.getElementById('stmt-result').innerHTML=html;
    };
  }
  ...
  async function loadSupplierStatementForm(){
    ...
    document.getElementById('tab-content').innerHTML=`<div class="card"><button class="btn btn-secondary btn-sm" onclick="loadReports()">🔙 رجوع</button><h3>كشف حساب مورد</h3><select class="select" id="supp-sel">${opts}</select><button class="btn btn-primary" id="btn-show-supp">عرض</button><div id="stmt-result"></div></div>`;
  ...
  async function loadDashboard(){
    ...
    document.getElementById('btn-export').onclick=async ()=>{
      const data={}; for(const t of ['items','customers','suppliers','categories','units','invoices','invoiceLines','payments','expenses']) data[t]=await getTable(t).toArray();
      const blob=new Blob([JSON.stringify(data)],{type:'application/json'}); const a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='alrajhi-backup.json'; a.click();
    };
    document.getElementById('btn-import').onclick=()=>{ const inp=document.createElement('input'); inp.type='file'; inp.accept='.json'; inp.onchange=async e=>{ const file=e.target.files[0]; if(!file) return; const text=await file.text(); const data=JSON.parse(text); if(await confirmDialog('سيتم استبدال جميع البيانات. متابعة؟')){ for(const t of Object.keys(data)){ await getTable(t).clear(); for(const row of data[t]) await getTable(t).add(row); } showToast('تم الاستيراد','success'); loadDashboard(); } }; inp.click(); };
  }

  // ===== التنقل والتهيئة =====
  const tabsConfig={
    dashboard:{title:'لوحة التحكم',icon:ICONS.home}, items:{title:'المواد',icon:ICONS.box},
    'sale-invoice':{title:'فاتورة بيع',icon:ICONS.cart}, 'purchase-invoice':{title:'فاتورة شراء',icon:ICONS.download},
    customers:{title:'العملاء',icon:ICONS.users}, suppliers:{title:'الموردين',icon:ICONS.factory},
    categories:{title:'التصنيفات',icon:ICONS.tag}, units:{title:'الوحدات',icon:ICONS.scale},
    payments:{title:'الدفعات',icon:ICONS.wallet}, expenses:{title:'المصاريف',icon:ICONS.dollar},
    invoices:{title:'الفواتير',icon:ICONS.fileText}, reports:{title:'التقارير',icon:ICONS.chart}
  };

  function setActiveTab(tab){ document.querySelectorAll('.nav-item,.bottom-item').forEach(el=>el.classList.toggle('active',el.dataset.tab===tab)); document.getElementById('page-title').textContent=tabsConfig[tab]?.title||''; }
  function navigateTo(tab){
    setActiveTab(tab); document.getElementById('more-menu').style.display='none'; document.getElementById('sidebar').classList.remove('open');
    const content=document.getElementById('tab-content'); content.style.opacity='0';
    setTimeout(async ()=>{
      switch(tab){
      ...
      content.style.transition='opacity 0.3s'; content.style.opacity='1';
    },50);
  }
  ...
  function initNavigation(){
    ...
    mainTabs.forEach(key=>{ const cfg=tabsConfig[key]; const btn=document.createElement('button'); btn.className='nav-item'+(key==='dashboard'?' active':''); btn.dataset.tab=key; btn.innerHTML=`${cfg.icon} <span>${cfg.title}</span>`; btn.onclick=()=>navigateTo(key); sidebarNav.appendChild(btn); });
    moreTabs.forEach(key=>{ const cfg=tabsConfig[key]; const btn=document.createElement('button'); btn.className='sheet-item'; btn.dataset.tab=key; btn.innerHTML=`${cfg.icon} <span>${cfg.title}</span>`; btn.onclick=()=>{ unlockScroll(); navigateTo(key); }; sheetGrid.appendChild(btn); });
  }
  ...
  document.querySelectorAll('.bottom-item').forEach(b=>b.addEventListener('click',()=>{ const tab=b.dataset.tab; if(tab==='more'){ document.getElementById('more-menu').style.display='flex'; lockScroll(); } else if(tab) navigateTo(tab); }));
  ...
  async function initApp(){
    try{
      [itemsCache,customersCache,suppliersCache,invoicesCache,categoriesCache,unitsCache]=await Promise.all([
        apiCall('/items','GET'),apiCall('/customers','GET'),apiCall('/suppliers','GET'),apiCall('/invoices','GET'),apiCall('/definitions?type=category','GET'),apiCall('/definitions?type=unit','GET')
      ]);
      initNavigation();
      loadDashboard();
      document.getElementById('loading-screen').classList.add('hidden');
    }catch(e){ showToast(e.message,'error'); document.getElementById('loading-screen').classList.add('hidden'); }
  }
  initApp();
})();
