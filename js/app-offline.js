...
window.printInvoice = function(invoice, options={}){
  ...
  // دالة مساعدة لإضافة زر الرجوع بطريقة آمنة
  function addBackButton(title, targetTab = 'reports') {
    const tc = document.getElementById('tab-content');
    if (tc) {
      // إدراج زر الرجوع في بداية المحتوى
      const backBtn = document.createElement('button');
      backBtn.className = 'btn btn-secondary btn-sm';
      backBtn.style.marginBottom = '12px';
      backBtn.textContent = '🔙 رجوع';
      backBtn.addEventListener('click', () => navigateTo(targetTab));
      // نضيفه قبل أي عنصر آخر إذا كان هناك عنصر موجود، وإلا سنحتاج لبناء HTML مناسب
      // للتبسيط، سنقوم في كل تقرير ببناء HTML يتضمن الزر مع id محدد ثم نربطه، كما هو موضح أدناه.
      // هذه الدالة توفر طريقة موحدة لكننا سنستخدم الطريقة المباشرة داخل كل دالة تقرير.
    }
  }
...
// ===== لوحة التحكم (مع تصدير واستيراد محسّنين) =====
...
async function handleImport(file) {
  ...
  try {
    ...
    if (tables.length === 0) throw new Error('الملف فارغ');
    ...
    const tableDetails = tables.map(t => {
      ...
      return `<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--border);">
        <span>${t}</span>
        <span style="font-weight:700;">${count} سجل</span>
      </div>`;
    }).join('');
    ...
    const modal = openModal({
      ...
      bodyHTML: `
        <p style="margin-bottom:12px;">تم العثور على <strong>${tables.length} جداول</strong> تحتوي على <strong>${totalRecords} سجل</strong>.</p>
        <div style="background:var(--bg);border-radius:12px;padding:12px;margin-bottom:16px;">
          ${tableDetails}
        </div>
        <p style="color:var(--danger);font-size:14px;">⚠️ تحذير: سيتم <strong>مسح جميع بياناتك الحالية</strong> واستبدالها بمحتويات هذا الملف.</p>
        <p style="font-size:13px;color:var(--text-muted);margin-top:8px;">تأكد من أن الملف من مصدر موثوق.</p>
      `,
      footerHTML: `
    ...
    modal.element.querySelector('#imp-confirm').onclick = async () => {
      ...
      // إعادة تحميل الكاشات وعرض لوحة التحكم
